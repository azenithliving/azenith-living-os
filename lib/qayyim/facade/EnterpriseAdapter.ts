...
export function toEnterpriseKey(raw: string): EnterpriseKey {
  ...
  if (k === "prime" || k === "qayyim-core" || k === "qayyim") return "qayyim";
...
/**
 * يجمع أداء qayyim-core + 7 subs في صف واحد للعرض في /admin/v2 (Enterprise)
 */
export async function getEnterpriseAgentsPerformance() {
  ...
  const qayyimIds = [idByKey.get("qayyim-core"), ...QAYYIM_ENTERPRISE_META.subAgents.map(k => idByKey.get(k))].filter(Boolean) as string[];
  ...
  for (const [k, id] of idByKey.entries()) {
    if (k === "qayyim-core" || QAYYIM_ENTERPRISE_META.subAgents.includes(k)) entKeyByProfileId.set(id, "qayyim");
