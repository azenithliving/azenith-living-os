...
import { roundFreshness, ROUND_STALE_MS } from "@/lib/ops/round-freshness";
...
export async function executeDailyRound(opts: { only?: RoundStep } = {}): Promise<DailyRoundOutcome> {
  ...
  const canaryStep = async () => {
    try {
      const { runCanary } = await import("@/lib/ops/canary");
  ...
  const auditStep = async () => {
    try {
      const { runSelfAudit } = await import("@/lib/ops/self-audit");
  ...
  try {
    const { scanTrafficAnomalies, renderAnomalyDigest } = await import("@/lib/ops/anomaly");
  ...
  try {
    const { reconcileStaleTasks } = await import("@/lib/ops/task-reconcile");
