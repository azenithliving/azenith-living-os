...
import { twoProportionZTest } from "@/lib/ops/stats";
import { verdictForExperiment, type ExperimentVerdict } from "@/lib/ops/experiment-verdict";
...
export async function createExperiment(input: CreateExperimentInput): Promise<Experiment> {
  const { data, error } = await supabaseServer
    ...
    .insert({
      ...
      status: 'draft',
      created_by: input.created_by || 'ops-ux',
