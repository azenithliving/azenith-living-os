...
const AGENT_CAPABILITIES: Record<string, string[]> = {
  'ops-lead': ['coordination', 'audit_full_site', 'publish', 'rollback', 'quality_gate'],
  'ops-content': ['draft_copy', 'unify_tone', 'identity_check', 'arabic_polish', 'copy_review'],
  'ops-visual': ['curate_gallery', 'select_hero_image', 'generate_alt_text', 'brand_consistency_check', 'image_optimization', 'gallery_sequencing'],
  'ops-seo': ['audit_seo', 'fix_seo', 'generate_schema', 'content_gap', 'keyword_research'],
  'ops-ux': ['analyze_behavior', 'design_ab_test', 'create_goal', 'exit_rate_report'],
  'ops-analytics': ['revenue_correlation', 'predict_impact', 'luxury_score', 'segment_luxury_buyers', 'weekly_report'],
  'ops-dev': ['code_review', 'bundle_analysis', 'dependency_audit', 'performance_budgets', 'security_code_scan'],
  'ops-qa': ['full_qa_suite', 'e2e_smoke', 'visual_regression', 'accessibility_audit', 'load_test', 'security_scan', 'cross_browser'],
};
...
export class MasterOrchestrator {
  ...
  private async aggregateNode(state: SwarmStateType): Promise<Partial<SwarmStateType>> {
    ...
    const coreAgent = AGENT_REGISTRY['ops-lead'];
  ...
  private async qualityGateNode(state: SwarmStateType): Promise<Partial<SwarmStateType>> {
    ...
    const coreAgent = AGENT_REGISTRY['ops-lead'];
    const qaAgent = AGENT_REGISTRY['ops-qa'];
...
الوكلاء المتاحون:
- ops-lead: تنسيق، تدقيق شامل، نشر، تراجع، جودة
- ops-content: محتوى عربي فاخر، توحيد نبرة، قانون هوية، صقل نصوص
- ops-visual: صور، هيرو، alt text، علامة تجارية، تحسين صور
- ops-seo: تدقيق SEO، إصلاح، Schema، فجوات محتوى، كلمات مفتاحية
- ops-ux: سلوك زائر، A/B tests، أهداف، تقارير خروج
- ops-analytics: ربط تحويل بإيرادات، تنبؤ، Luxury Score، تقسيم عملاء
- ops-dev: مراجعة كود، Bundle، تبعيات، أداء، أمان كود
- ops-qa: E2E، Visual Regression، a11y، Load Test، Security
