...
export class QayyimFacade {
  ...
  async getSwarmStatus(companyHint?: string | null): Promise<{
    ...
    if (!supabase || !companyId) {
      return { leader: { key: "ops-lead", status: "offline", lastActivity: null }, subs: [] };
    }
    ...
    const { data: profiles } = await supabase
      ...
      .in("agent_key", ["ops-lead", ...QAYYIM_ENTERPRISE_META.subAgents])
    ...
    const leaderId = idByKey.get("ops-lead");
