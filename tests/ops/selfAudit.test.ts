...
describe('pickAuditSamples', () => {
  ...
  it('writes the agent technical key, not the display form', () => {
    const rows = [
      ...
      msg({ sender_name: 'OPS-ANALYTICS', content: 'المؤشرات اللحظية: الطلبات اتنين والمخزون متوفر في كل الأقسام.' }),
    ];
    expect(pickAuditSamples(rows)[0].agentKey).toBe('ops-analytics');
  });

  it('resolves a stamp a bundle cached before the rename left behind', () => {
    const rows = [
      msg({ sender_type: 'user', content: 'اعرضلي المؤشرات اللحظية دلوقتي' }),
  ...
  it('falls back to the swarm key when the sender is not a key', () => {
    ...
    expect(pickAuditSamples(rows).map((s) => s.agentKey)).toEqual(['ops-lead', 'ops-lead']);
  });
});
