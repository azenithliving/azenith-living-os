...
import { isRealPath, toPath } from '@/lib/qayyim/url-manifest';
import { deriveActions } from '@/lib/qayyim/chat-brain';
import { shouldDebate } from '@/lib/qayyim/debate';
import { constitutionEngine } from '@/lib/qayyim/governance/ConstitutionEngine';
import { explainGap } from '@/lib/qayyim/gap-contract';
import { holtWinters, twoProportionZTest } from '@/lib/qayyim/stats';
...
const ROUTING = [
  caseOf('hamza: what do you do', ['انت بتعمل إيه؟', 'qayyim_whoami']),
  caseOf('no hamza: what do you do', ['انت بتعمل ايه؟', 'qayyim_whoami']),
  caseOf('know your limits', ['قدراتك إيه', 'qayyim_whoami']),
  caseOf('introduce yourself', ['عرف نفسك', 'qayyim_whoami']),
  caseOf('who are you (english)', ['who are you', 'qayyim_whoami']),
  caseOf('forecast a month', ['توقع مبيعات الشهر الجاي', 'qayyim_forecast']),
  caseOf('forecast a week', ['توقع الأسبوع الجاي', 'qayyim_forecast']),
  caseOf('shop trend', ['ايزاي الشغل الفترة دي', 'qayyim_world']),
  caseOf('what sells', ['اللي بيبيع دلوقتي', 'qayyim_world']),
  caseOf('search phrases', ['كلمات البحث اللي جت زوار', 'gsc_queries']),
  caseOf('competitors', ['المنافسين نزلوا تشكيلة جديدة', 'qayyim_rivals']),
  ...
  caseOf('luxury index', ['مؤشر الفخامة كام', 'qayyim_luxury_score']),
  caseOf('goal at risk', ['أي هدف مهدد', 'qayyim_goals_risk']),
...
const INTENT_JSON = [
  caseOf('whitelisted tool passes', ['{"toolName":"qayyim_whoami","params":{}}', 'qayyim_whoami']),
  ...
  caseOf('prefix of a real tool rejected', ['{"toolName":"qayyim_whoami_all","params":{}}', null]),
...
const PATHS = [
  ...
  caseOf('admin section the owner uses', ['/admin/v2/agents/qayyim', true]),
